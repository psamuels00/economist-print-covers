...
async function getImage(year, ymd, thumbnailImageUrl, type, displayDate, isDateDisplayed) {
...
async function getIssueImages(year, issue, images, useRemoteImages) {
    const { ymd, thumbnailImageUrl, displayDate } = issue;
    let isDateDisplayed = false;  // used to ensure date is only displayed once

    let urls = {};
    for (const type of Object.keys(config.imagePath)) {
        const result = await thisModule.getImage(year, ymd, thumbnailImageUrl, type, displayDate, isDateDisplayed)
                      .catch((e) => {util.display('@@@', e.toString());});
        const { file, url } = result;
        isDateDisplayed = result.isDateDisplayed;
